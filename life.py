...
panSpeed = 0.1  # speed of panning (2 doubles the seed and 0.5 halfs the dafualt speed)
...
def configHandling(filename: str = "config.ini", conf: list = []) -> Any:
    global cameraPos, cellSize, panSpeed, field, screenSize, fieldSize
    if len(conf) == 0:
        try:
            ...
            configHandling(conf=json.dumps({"cameraPos": cameraPos, "cellSize": cellSize, "panSpeed": panSpeed, "field": field, "screenSize": screenSize, "fieldSize": fieldSize}))
    else:
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                configHandling(conf={"cameraPos": cameraPos, "cellSize": cellSize, "panSpeed": panSpeed, "field": field, "screenSize": screenSize, "fieldSize": fieldSize})
                pygame.quit()
                exit()
            # creating a cell
